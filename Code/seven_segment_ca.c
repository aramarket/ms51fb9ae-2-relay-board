...
// Lookup table for numbers 0-9 in 7-segment display format
...
void display_digit(unsigned char display, unsigned char digit, unsigned char value) {
    ...
    DIGIT1_A = 0; DIGIT2_A = 0; DIGIT3_A = 0;
    DIGIT1_B = 0; DIGIT2_B = 0; DIGIT3_B = 0;
    ...
    if (display == 1) {
        if (digit == 1) DIGIT1_A = 1;
        if (digit == 2) DIGIT2_A = 1;
        if (digit == 3) DIGIT3_A = 1;
    } else if (display == 2) {
        if (digit == 1) DIGIT1_B = 1;
        if (digit == 2) DIGIT2_B = 1;
        if (digit == 3) DIGIT3_B = 1;
    }
